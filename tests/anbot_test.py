from anbot.analyze import analyze_sticks, is_almost_two_identical_groups, is_one_group_left, is_only_singles_left, is_parity_state, is_parity_even, is_two_identical_groups_and_one_other, is_one_little_group_and_one_big_group, is_even_number_of_singles
from anbot.think import get_biggest_group_between_two, get_start_of_group, get_group_in_parity_state, get_index_of_first_single, get_group_different_from_the_others, remove_all_singles
...
def test_remove_all_singles():
    # No singles to remove
    assert remove_all_singles([2, 3, 4]) == [2, 3, 4]
    assert remove_all_singles([5]) == [5]
    assert remove_all_singles([]) == []

    # Only singles
    assert remove_all_singles([1, 1, 1]) == []
    assert remove_all_singles([1]) == []

    # Mixed singles and groups
    assert remove_all_singles([1, 2, 1, 3, 1]) == [2, 3]
    assert remove_all_singles([2, 1, 1, 4]) == [2, 4]
    assert remove_all_singles([1, 2, 3, 1]) == [2, 3]
    assert remove_all_singles([1, 1, 2, 1, 1]) == [2]
    assert remove_all_singles([1, 2, 1, 1, 3, 1]) == [2, 3]
